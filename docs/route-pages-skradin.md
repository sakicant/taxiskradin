# Skradin Route Pages Master List (English)

Every route STARTING at Skradin or Roški Slap (all matrix destinations over EUR 20),
plus the opposite direction of each EXCEPT where that page already exists on
taxisibenik.hr (so the two sites never duplicate a route). Prices come from the
PRICES matrix in script.js. Plastovo and the winery destinations are omitted (no
matrix fare; wineries are covered by /skradin-winery-transfers/). Generated batch.

| From | To | EN slug | One-way EUR |
|---|---|---|---|
| Skradin | ACI Marina Jezera | `taxi-skradin-to-aci-marina-jezera` | 70 |
| ACI Marina Jezera | Skradin | `taxi-aci-marina-jezera-to-skradin` | 70 |
| Skradin | ACI Marina Trogir | `taxi-skradin-to-aci-marina-trogir` | 115 |
| ACI Marina Trogir | Skradin | `taxi-aci-marina-trogir-to-skradin` | 115 |
| Skradin | ACI Marina Vodice | `taxi-skradin-to-aci-marina-vodice` | 60 |
| ACI Marina Vodice | Skradin | `taxi-aci-marina-vodice-to-skradin` | 60 |
| Skradin | Betina | `taxi-skradin-to-betina` | 75 |
| Betina | Skradin | `taxi-betina-to-skradin` | 75 |
| Skradin | Bilice | `taxi-skradin-to-bilice` | 40 |
| Bilice | Skradin | `taxi-bilice-to-skradin` | 40 |
| Skradin | Bilo | `taxi-skradin-to-bilo` | 85 |
| Bilo | Skradin | `taxi-bilo-to-skradin` | 85 |
| Skradin | Brodarica | `taxi-skradin-to-brodarica` | 55 |
| Brodarica | Skradin | `taxi-brodarica-to-skradin` | 55 |
| Skradin | Drniš | `taxi-skradin-to-drnis` | 65 |
| Drniš | Skradin | `taxi-drnis-to-skradin` | 65 |
| Skradin | Dubrovnik | `taxi-skradin-to-dubrovnik` | 485 |
| Dubrovnik | Skradin | `taxi-dubrovnik-to-skradin` | 485 |
| Skradin | Dubrovnik Airport | `taxi-skradin-to-dubrovnik-airport` | 485 |
| Dubrovnik Airport | Skradin | `taxi-dubrovnik-airport-to-skradin` | 485 |
| Skradin | Grebaštica | `taxi-skradin-to-grebastica` | 80 |
| Grebaštica | Skradin | `taxi-grebastica-to-skradin` | 80 |
| Skradin | Jadrija | `taxi-skradin-to-jadrija` | 75 |
| Jadrija | Skradin | `taxi-jadrija-to-skradin` | 75 |
| Skradin | Jezera | `taxi-skradin-to-jezera` | 70 |
| Jezera | Skradin | `taxi-jezera-to-skradin` | 70 |
| Skradin | Knin | `taxi-skradin-to-knin` | 95 |
| Knin | Skradin | `taxi-knin-to-skradin` | 95 |
| Skradin | Lozovac | `taxi-skradin-to-lozovac` | 25 |
| Lozovac | Skradin | `taxi-lozovac-to-skradin` | 25 |
| Skradin | Makarska | `taxi-skradin-to-makarska` | 210 |
| Makarska | Skradin | `taxi-makarska-to-skradin` | 210 |
| Skradin | Marina Agana | `taxi-skradin-to-marina-agana` | 100 |
| Marina Agana | Skradin | `taxi-marina-agana-to-skradin` | 100 |
| Skradin | Marina Baotić | `taxi-skradin-to-marina-baotic` | 100 |
| Marina Baotić | Skradin | `taxi-marina-baotic-to-skradin` | 100 |
| Skradin | Marina Betina | `taxi-skradin-to-marina-betina` | 75 |
| Marina Betina | Skradin | `taxi-marina-betina-to-skradin` | 75 |
| Skradin | Marina Frapa | `taxi-skradin-to-marina-frapa` | 90 |
| Marina Frapa | Skradin | `taxi-marina-frapa-to-skradin` | 90 |
| Skradin | Marina Hramina | `taxi-skradin-to-marina-hramina` | 75 |
| Marina Hramina | Skradin | `taxi-marina-hramina-to-skradin` | 75 |
| Skradin | Marina Kremik | `taxi-skradin-to-marina-kremik` | 85 |
| Marina Kremik | Skradin | `taxi-marina-kremik-to-skradin` | 85 |
| Skradin | Marina Tribunj | `taxi-skradin-to-marina-tribunj` | 60 |
| Marina Tribunj | Skradin | `taxi-marina-tribunj-to-skradin` | 60 |
| Skradin | Marina Trogir (SCT) | `taxi-skradin-to-marina-trogir-sct` | 115 |
| Marina Trogir (SCT) | Skradin | `taxi-marina-trogir-sct-to-skradin` | 115 |
| Skradin | Murter | `taxi-skradin-to-murter` | 75 |
| Murter | Skradin | `taxi-murter-to-skradin` | 75 |
| Skradin | Perković | `taxi-skradin-to-perkovic` | 60 |
| Perković | Skradin | `taxi-perkovic-to-skradin` | 60 |
| Skradin | Pirovac | `taxi-skradin-to-pirovac` | 65 |
| Pirovac | Skradin | `taxi-pirovac-to-skradin` | 65 |
| Skradin | Primošten | `taxi-skradin-to-primosten` | 80 |
| Primošten | Skradin | `taxi-primosten-to-skradin` | 80 |
| Skradin | Rogoznica | `taxi-skradin-to-rogoznica` | 90 |
| Rogoznica | Skradin | `taxi-rogoznica-to-skradin` | 90 |
| Skradin | Seget | `taxi-skradin-to-seget` | 100 |
| Seget | Skradin | `taxi-seget-to-skradin` | 100 |
| Skradin | Split | `taxi-skradin-to-split` | 140 |
| Split | Skradin | `taxi-split-to-skradin` | 140 |
| Skradin | Split Airport | `taxi-skradin-to-split-airport` | 110 |
| Split Airport | Skradin | `taxi-split-airport-to-skradin` | 110 |
| Skradin | Srima | `taxi-skradin-to-srima` | 65 |
| Srima | Skradin | `taxi-srima-to-skradin` | 65 |
| Skradin | Tisno | `taxi-skradin-to-tisno` | 65 |
| Tisno | Skradin | `taxi-tisno-to-skradin` | 65 |
| Skradin | Tribunj | `taxi-skradin-to-tribunj` | 60 |
| Tribunj | Skradin | `taxi-tribunj-to-skradin` | 60 |
| Skradin | Trogir | `taxi-skradin-to-trogir` | 100 |
| Trogir | Skradin | `taxi-trogir-to-skradin` | 100 |
| Skradin | Vodice | `taxi-skradin-to-vodice` | 60 |
| Vodice | Skradin | `taxi-vodice-to-skradin` | 60 |
| Skradin | Zablaće | `taxi-skradin-to-zablace` | 50 |
| Zablaće | Skradin | `taxi-zablace-to-skradin` | 50 |
| Skradin | Zadar | `taxi-skradin-to-zadar` | 130 |
| Zadar | Skradin | `taxi-zadar-to-skradin` | 130 |
| Skradin | Zadar Airport | `taxi-skradin-to-zadar-airport` | 105 |
| Zadar Airport | Skradin | `taxi-zadar-airport-to-skradin` | 105 |
| Skradin | Zagreb | `taxi-skradin-to-zagreb` | 485 |
| Zagreb | Skradin | `taxi-zagreb-to-skradin` | 485 |
| Skradin | Zagreb Airport | `taxi-skradin-to-zagreb-airport` | 485 |
| Zagreb Airport | Skradin | `taxi-zagreb-airport-to-skradin` | 485 |
| Skradin | Zaton | `taxi-skradin-to-zaton` | 60 |
| Zaton | Skradin | `taxi-zaton-to-skradin` | 60 |
| Skradin | Čiovo | `taxi-skradin-to-ciovo` | 115 |
| Čiovo | Skradin | `taxi-ciovo-to-skradin` | 115 |
| Skradin | Šibenik | `taxi-skradin-to-sibenik` | 50 |
| Skradin | Žaborić | `taxi-skradin-to-zaboric` | 75 |
| Žaborić | Skradin | `taxi-zaboric-to-skradin` | 75 |
| Roški Slap | ACI Marina Jezera | `taxi-roski-slap-to-aci-marina-jezera` | 95 |
| ACI Marina Jezera | Roški Slap | `taxi-aci-marina-jezera-to-roski-slap` | 95 |
| Roški Slap | ACI Marina Trogir | `taxi-roski-slap-to-aci-marina-trogir` | 125 |
| ACI Marina Trogir | Roški Slap | `taxi-aci-marina-trogir-to-roski-slap` | 125 |
| Roški Slap | ACI Marina Vodice | `taxi-roski-slap-to-aci-marina-vodice` | 90 |
| ACI Marina Vodice | Roški Slap | `taxi-aci-marina-vodice-to-roski-slap` | 90 |
| Roški Slap | Betina | `taxi-roski-slap-to-betina` | 115 |
| Betina | Roški Slap | `taxi-betina-to-roski-slap` | 115 |
| Roški Slap | Bilice | `taxi-roski-slap-to-bilice` | 80 |
| Bilice | Roški Slap | `taxi-bilice-to-roski-slap` | 80 |
| Roški Slap | Bilo | `taxi-roski-slap-to-bilo` | 125 |
| Bilo | Roški Slap | `taxi-bilo-to-roski-slap` | 125 |
| Roški Slap | Brodarica | `taxi-roski-slap-to-brodarica` | 80 |
| Brodarica | Roški Slap | `taxi-brodarica-to-roski-slap` | 80 |
| Roški Slap | Dubrovnik | `taxi-roski-slap-to-dubrovnik` | 485 |
| Dubrovnik | Roški Slap | `taxi-dubrovnik-to-roski-slap` | 485 |
| Roški Slap | Dubrovnik Airport | `taxi-roski-slap-to-dubrovnik-airport` | 485 |
| Dubrovnik Airport | Roški Slap | `taxi-dubrovnik-airport-to-roski-slap` | 485 |
| Roški Slap | Grebaštica | `taxi-roski-slap-to-grebastica` | 90 |
| Grebaštica | Roški Slap | `taxi-grebastica-to-roski-slap` | 90 |
| Roški Slap | Jadrija | `taxi-roski-slap-to-jadrija` | 100 |
| Jadrija | Roški Slap | `taxi-jadrija-to-roski-slap` | 100 |
| Roški Slap | Jezera | `taxi-roski-slap-to-jezera` | 95 |
| Jezera | Roški Slap | `taxi-jezera-to-roski-slap` | 95 |
| Roški Slap | Lozovac | `taxi-roski-slap-to-lozovac` | 65 |
| Lozovac | Roški Slap | `taxi-lozovac-to-roski-slap` | 65 |
| Roški Slap | Marina Agana | `taxi-roski-slap-to-marina-agana` | 110 |
| Marina Agana | Roški Slap | `taxi-marina-agana-to-roski-slap` | 110 |
| Roški Slap | Marina Baotić | `taxi-roski-slap-to-marina-baotic` | 110 |
| Marina Baotić | Roški Slap | `taxi-marina-baotic-to-roski-slap` | 110 |
| Roški Slap | Marina Betina | `taxi-roski-slap-to-marina-betina` | 115 |
| Marina Betina | Roški Slap | `taxi-marina-betina-to-roski-slap` | 115 |
| Roški Slap | Marina Frapa | `taxi-roski-slap-to-marina-frapa` | 120 |
| Marina Frapa | Roški Slap | `taxi-marina-frapa-to-roski-slap` | 120 |
| Roški Slap | Marina Hramina | `taxi-roski-slap-to-marina-hramina` | 115 |
| Marina Hramina | Roški Slap | `taxi-marina-hramina-to-roski-slap` | 115 |
| Roški Slap | Marina Kremik | `taxi-roski-slap-to-marina-kremik` | 115 |
| Marina Kremik | Roški Slap | `taxi-marina-kremik-to-roski-slap` | 115 |
| Roški Slap | Marina Tribunj | `taxi-roski-slap-to-marina-tribunj` | 95 |
| Marina Tribunj | Roški Slap | `taxi-marina-tribunj-to-roski-slap` | 95 |
| Roški Slap | Marina Trogir (SCT) | `taxi-roski-slap-to-marina-trogir-sct` | 125 |
| Marina Trogir (SCT) | Roški Slap | `taxi-marina-trogir-sct-to-roski-slap` | 125 |
| Roški Slap | Murter | `taxi-roski-slap-to-murter` | 115 |
| Murter | Roški Slap | `taxi-murter-to-roski-slap` | 115 |
| Roški Slap | Pirovac | `taxi-roski-slap-to-pirovac` | 85 |
| Pirovac | Roški Slap | `taxi-pirovac-to-roski-slap` | 85 |
| Roški Slap | Primošten | `taxi-roski-slap-to-primosten` | 110 |
| Primošten | Roški Slap | `taxi-primosten-to-roski-slap` | 110 |
| Roški Slap | Rogoznica | `taxi-roski-slap-to-rogoznica` | 120 |
| Rogoznica | Roški Slap | `taxi-rogoznica-to-roski-slap` | 120 |
| Roški Slap | Seget | `taxi-roski-slap-to-seget` | 110 |
| Seget | Roški Slap | `taxi-seget-to-roski-slap` | 110 |
| Roški Slap | Skradin | `taxi-roski-slap-to-skradin` | 50 |
| Skradin | Roški Slap | `taxi-skradin-to-roski-slap` | 50 |
| Roški Slap | Split | `taxi-roski-slap-to-split` | 150 |
| Split | Roški Slap | `taxi-split-to-roski-slap` | 150 |
| Roški Slap | Split Airport | `taxi-roski-slap-to-split-airport` | 100 |
| Split Airport | Roški Slap | `taxi-split-airport-to-roski-slap` | 100 |
| Roški Slap | Srima | `taxi-roski-slap-to-srima` | 90 |
| Srima | Roški Slap | `taxi-srima-to-roski-slap` | 90 |
| Roški Slap | Tisno | `taxi-roski-slap-to-tisno` | 90 |
| Tisno | Roški Slap | `taxi-tisno-to-roski-slap` | 90 |
| Roški Slap | Tribunj | `taxi-roski-slap-to-tribunj` | 95 |
| Tribunj | Roški Slap | `taxi-tribunj-to-roski-slap` | 95 |
| Roški Slap | Trogir | `taxi-roski-slap-to-trogir` | 110 |
| Trogir | Roški Slap | `taxi-trogir-to-roski-slap` | 110 |
| Roški Slap | Tromilja | `taxi-roski-slap-to-tromilja` | 60 |
| Tromilja | Roški Slap | `taxi-tromilja-to-roski-slap` | 60 |
| Roški Slap | Vodice | `taxi-roski-slap-to-vodice` | 90 |
| Vodice | Roški Slap | `taxi-vodice-to-roski-slap` | 90 |
| Roški Slap | Zablaće | `taxi-roski-slap-to-zablace` | 90 |
| Zablaće | Roški Slap | `taxi-zablace-to-roski-slap` | 90 |
| Roški Slap | Zadar | `taxi-roski-slap-to-zadar` | 135 |
| Zadar | Roški Slap | `taxi-zadar-to-roski-slap` | 135 |
| Roški Slap | Zadar Airport | `taxi-roski-slap-to-zadar-airport` | 110 |
| Zadar Airport | Roški Slap | `taxi-zadar-airport-to-roski-slap` | 110 |
| Roški Slap | Zagreb | `taxi-roski-slap-to-zagreb` | 485 |
| Zagreb | Roški Slap | `taxi-zagreb-to-roski-slap` | 485 |
| Roški Slap | Zagreb Airport | `taxi-roski-slap-to-zagreb-airport` | 485 |
| Zagreb Airport | Roški Slap | `taxi-zagreb-airport-to-roski-slap` | 485 |
| Roški Slap | Zaton | `taxi-roski-slap-to-zaton` | 75 |
| Zaton | Roški Slap | `taxi-zaton-to-roski-slap` | 75 |
| Roški Slap | Čiovo | `taxi-roski-slap-to-ciovo` | 125 |
| Čiovo | Roški Slap | `taxi-ciovo-to-roski-slap` | 125 |
| Roški Slap | Šibenik | `taxi-roski-slap-to-sibenik` | 90 |
| Šibenik | Roški Slap | `taxi-sibenik-to-roski-slap` | 90 |
| Roški Slap | Žaborić | `taxi-roski-slap-to-zaboric` | 115 |
| Žaborić | Roški Slap | `taxi-zaboric-to-roski-slap` | 115 |
| Skradin | Bibich Winery | `taxi-skradin-to-bibich-winery` | 40 |
| Bibich Winery | Skradin | `taxi-bibich-winery-to-skradin` | 40 |
| Skradin | Sladić Winery | `taxi-skradin-to-sladic-winery` | 40 |
| Sladić Winery | Skradin | `taxi-sladic-winery-to-skradin` | 40 |
| Roški Slap | Bibich Winery | `taxi-roski-slap-to-bibich-winery` | 50 |
| Bibich Winery | Roški Slap | `taxi-bibich-winery-to-roski-slap` | 50 |
| Roški Slap | Sladić Winery | `taxi-roski-slap-to-sladic-winery` | 50 |
| Sladić Winery | Roški Slap | `taxi-sladic-winery-to-roski-slap` | 50 |
| Skradin | Novalja | `taxi-skradin-to-novalja` | 190 |
| Novalja | Skradin | `taxi-novalja-to-skradin` | 190 |
| Roški Slap | Novalja | `taxi-roski-slap-to-novalja` | 190 |
| Novalja | Roški Slap | `taxi-novalja-to-roski-slap` | 190 |
